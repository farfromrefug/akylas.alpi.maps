module.exports = function(gulp, plugins) {
    gulp.task('install:android_sdk', ['install:create_emulator'], function() {
        plugins.utils.log(plugins.utils.colors.cyan.bold(
            "Yeay ! Android SDK have been successfully installed."));
    });
    ...
    gulp.task('install:update_arm', ['install:update_addOnGoogle'], function(done) {
        plugins.exec('echo yes | android -s update sdk --no-ui --all --filter sys-img-armeabi-v7a-android-' +
            process.env.ANDROID_VERSION, done).stdout.pipe(process.stdout);
    });

    gulp.task('install:update_addOnGoogle', ['install:update_android'], function(done) {
        plugins.exec('echo yes | android -s update sdk --no-ui --all --filter addon-google_apis-google-' +
            process.env.ANDROID_VERSION, done).stdout.pipe(process.stdout);
    });

    gulp.task('install:update_android', ['install:update_extraSupport'], function(done) {
        plugins.exec('echo yes | android -s update sdk --no-ui --all --filter android-' + process.env.ANDROID_VERSION,
            done).stdout.pipe(process.stdout);
    });

    gulp.task('install:update_extraSupport', ['install:update_buildTools'], function(done) {
        plugins.exec('echo yes | android -s update sdk --no-ui --all --filter extra-android-support', done)
            .stdout.pipe(process.stdout);
    });

    gulp.task('install:update_buildTools', ['install:update_tools'], function(done) {
        plugins.exec('echo yes | android -s update sdk --no-ui --all --filter build-tools-', done).stdout.pipe(
            process.stdout);
    });

    gulp.task('install:update_tools', ['install:update_platformTools'], function(done) {
        plugins.exec('echo yes | android -s update sdk --no-ui --all --filter tools', done).stdout.pipe(
            process.stdout);
    });

    gulp.task('install:update_platformTools', ['install:configPath'], function(done) {
        plugins.exec('echo yes | android -s update sdk --no-ui --all --filter platform-tools', done).stdout
            .pipe(process.stdout);
    });
    ...
    gulp.task('install:configPath', ['install:getSDK'], function(done) {
        plugins.exec('titanium config android.sdkPath $ANDROID_HOME', function(err) {
            ...
            plugins.utils.log(plugins.utils.colors.cyan.bold("Processing install for android " +
    ...
    gulp.task('start:emulator', ['ensure:android'], function(done) {
        ...
        var waitForEmulatorToStart = function(nbTries, maxTries, callback) {
            ...
            plugins.exec('adb -e shell getprop init.svc.bootanim', function(err, stdout) {
                if (nbTries < maxTries && (stdout === null || stdout.match(/stopped/) === null)) {
                    setTimeout(waitForEmulatorToStart, 1000 * 30, nbTries + 1, maxTries,
                        callback);
        ...
        var unlockScreen = function(nbTries, maxTries, callback) {
            ...
            plugins.exec('adb -e shell input keyevent 82', function(err, stdout) {
                plugins.exec('adb -e shell dumpsys window windows | grep mCurrentFocus',
                    function(err, stdout) {
                        if (nbTries < maxTries && stdout.match(/Keyguard/) !== null) {
                            setTimeout(unlockScreen, 1000 * 10, nbTries + 1, maxTries,
                                callback);
                        } else if (nbTries >= maxTries) {
    ...
    gulp.task('ensure:android', function() {
        if (process.env.PLATFORM !== "android") {
            plugins.utils.log(plugins.utils.colors.yellow.bold(
                "Android is not the target platform; task skipped..."));
            process.exit();
        }
    });
