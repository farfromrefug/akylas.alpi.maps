...
export class Geolib {
    ...
    static XDPI = (Ti.Platform.displayCaps.xdpi || Ti.Platform.displayCaps.dpi) / Ti.Platform.displayCaps.logicalDensityFactor;
    static PX_PER_CM = Geolib.XDPI / 2.54;

    getNiceNumber(number) {
    ...
    get sexagesimalPattern() {
        return /^([0-9]{1,3})°\s*([0-9]{1,3}(?:\.(?:[0-9]{1,2}))?)'\s*(([0-9]{1,3}(\.([0-9]{1,2}))?)"\s*)?([NEOSW]?)$/;
    }
    measures = {
        m: 1,
        km: 0.001,
    ...
    getDistance=(start, end, accuracy?) =>{
        ...
        var distance: any = b * A * (sigma - deltaSigma);
    ...
    getDistanceSimple=(start, end, accuracy?) =>{
        ...
        var distance = Math.round(Math.acos(Math.sin(elat) * Math.sin(slat) + Math.cos(elat) * Math.cos(slat) * Math.cos(slon - elon)) * this.earthRadius);
    ...
    scaleBounds(_region, _deltaFactor) {
        if (_deltaFactor !== 0) {
            var delta = Math.max(_region.ne.latitude - _region.sw.latitude, _region.ne.longitude - _region.sw.longitude) * _deltaFactor;
    ...
    distanceStr = (_distance, _unit?, _factor?) => {
        ...
        var result = _distance * this.measures[_unit];
    ...
    speed(_speed, _unit?) {
        ...
        var result = 1000 * _speed * this.measures[_unit];
        if (result >= 100) {
            dec = 0;
        } else if (result < 10) {
            dec *= 2;
