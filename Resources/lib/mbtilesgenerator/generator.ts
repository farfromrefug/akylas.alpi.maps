  ...
  return Promise.resolve().then(()=> {
    ...
    //   }
      ...
      return createTables(request.db, [
        ...
        request.db.close();
        //move file
        request.db.file.move(DB_DIR + request.file);

        //get real file size now
        request.size = Ti.Filesystem.getFile(DB_DIR + request.file).size;

        console.debug('MBTile computed successfully. File output is available in ' + request.file);
        // Persist tile state
    ...
    request.db.close();
    if (e.message !== 'stopped') { //else already handled
      request.stop();
