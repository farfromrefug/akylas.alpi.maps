...
export class ItemHdlr extends EventEmitter {
    ...
    itemDetails = (_item: Item, _desc: ItemType) => {
        return this.convert.osmTagsFastDetails(_item)
            .join(', ');
    }
        ...
        if (!subtitle) {
            if (_item && _item.address) {
                ...
                if (isRoute) {
                    ...
                    subtitle = this.htmlIcon($.sDist, 0, $.black) + this.geolib.formatter.distance(
                        route.distance);
                    // if (route.duration > 0) {
                    ...
                    if (profile) {
                        if (profile.dplus) {
                            subtitle += ' ' + this.htmlIcon($.sDplus, 0, $.black) + this.geolib
                                .formatter
                                .altitude(profile.dplus);
                        }
                        if (profile.dmin) {
                            subtitle += ' ' + this.htmlIcon($.sDmin, 0, $.black) + this.geolib
                                .formatter
    ...
    getAddressSearchTermOSM(_item: Item) {
        ...
        var params = [
            ['address26', 'address29', 'footway', 'pedestrian', 'road'],
            ['city', 'town', 'village']
        ];
        var count = 0;
    ...
    updateCamera = (_mapView: MapView, _params: MapUpdateCamParams, _deltaScreen?: { top?, bottom?, left?, right?}) => {
    ...
    updateParamsForLocation(_item: Item, _location: Location) {
        var args: any = {};
        if (!_item) {
            return args;
        }
    ...
    infoRowItemForItem = (_item: Item, _itemDesc: ItemType, _defaults?, _currentLocation?: Location) => {
    ...
    handlePhotoTaken = (_parent: AppWindow, _callback: Function, e: TiEvent) => {
        _parent.showLoading({
            label: {
        ...
        Ti.Filesystem.getFile(this.getImagePath(imageName))
            .write(image);
    ...
    takePhoto = (_parent: AppWindow, _callback: (...args) => any) => {
    ...
    handleItemAction = (_option: string, _items: Item | Item[], _desc: ItemType, _callback: Function, _parent: AppWindow, _mapHandler: MapWindow, _params?) => {
        ...
        switch (_option) {
            ...
            case 'add_photo':
                {
                    this.handleItemAction('acquire_photo', _item, _desc, (option,
                        photo: {
                            width: number,
                            height: number,
                            image: string
                        ...
                        if (photo) {
                            let result = this.updateItem(_item, _desc, {
                                newPhotos: [photo]
                            }, _mapHandler);
                            if (_callback) {
                                _callback(_option, result, image);
                            }
                        }
                    }, _parent, _mapHandler);
                    break;
            ...
            case 'acquire_photo':
                {
                    var onSuccess = (_image, _imageId, _imageName) => {
                        var result;
                        if (_imageId) {
                            result = {
                                width: _image.width,
            ...
            case 'refresh_tags':
                {
                    ...
                    var request = app.api.parallelMapRequests(calls).then(function (res) {
                        var callsToAdd = [];
                            ...
                            if (_.isObject(value)) {
                                if (value.photos) {
                                    _.forEach(value.photos, function (photo) {
                                        ...
                                        var existing = _.findIndex(_item.photos, { 'url ': url });
                                        if (existing === -1) {
                                            hasChanged = true;
                                ...
                                if (value.notes) {
                                    _.forEach(value.notes, function (note) {
                                        var id = note.title;
                                        var existing = _.findIndex(_item.notes, { 'title': id });
                                        if (existing === -1) {
                                            hasChanged = true;
                    ...
                    }).catch(e => _parent && _parent.showError(e)).then((r) => { _parent && _parent.hideLoading(); return r; });
                    if (_parent) {
                        _parent.showLoading({
                            request: request,
                            label: {
            ...
            case 'facebook':
            case 'website':
                this.showFloatingWebView(_option, _item.tags[_option], _item, _desc, _parent,
                    _mapHandler);
                break;
            case 'search_google':
                {
            ...
            case 'reverse_geo':
                {
                    request = app.api.reverseGeocode(_item).then((res) => {
            ...
            case 'images':
                {
                    this.showFloatingWebView(_option, 'http://www.google.com/images?q=' + escape(this.itemSearchTerm(
                        _item)),
                        _item, _desc,
                        _parent, _mapHandler);
            ...
            case 'consolidate_alt':
                {
                    request = app.api.ignElevation(_item).then(changes => {
            ...
            case 'query_profile':
                {
                    ...
                    request = app.api.ignElevationProfile(this, route.points).then(
                        (changes) => {
            ...
            case 'move':
                {
                    var lists: ItemList[] = _.filter(_.map(_.flatten(_mapHandler.runGetMethodOnModules(
                        'getLists')),
