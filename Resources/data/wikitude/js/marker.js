...
function Marker(poiData) {
    ...
    */
    ...
    var cams = [];
    var onClick = Marker.prototype.getOnClickTrigger(this);
    // create an AR.Label for the marker's title 
    if (poiData.title) {
        this.titleLabel = new AR.Label(poiData.title.trunc(20), 1, {
            zOrder: 1,
            offsetY: 1.15,
            // verticalAnchor: AR.CONST.VERTICAL_ANCHOR.TOP,
            style: {
                backgroundColor: poiData.color,
                textColor: '#FFFFFF',
                fontStyle: AR.CONST.FONT_STYLE.BOLD
            },
            onClick: onClick
        });
        cams.push(this.titleLabel);
    }
    ...
    if (poiData.description) {
        ...
        cams.push(this.descriptionLabel);
    }

    // create an AR.ImageDrawable for the marker in idle state
    if (poiData.image) {
        if (!World.markerDrawables[poiData.image]) {
            World.markerDrawables[poiData.image] = new AR.ImageResource(poiData.image);
        }
        this.markerDrawable_idle = new AR.ImageDrawable(World.markerDrawables[poiData.image], 2.5, {
            zOrder: 0,
            opacity: 1.0,
            onClick: onClick
        });
        cams.push(this.markerDrawable_idle);
    } else {
        this.markerDrawable_idle = new AR.ImageDrawable(World.markerDrawables['default'], 2.5, {
            zOrder: 0,
            opacity: 1.0,
            onClick: onClick
        });
    ...
    if (poiData.selectedImage) {
        if (!World.markerDrawables[poiData.selectedImage]) {
            World.markerDrawables[poiData.selectedImage] = new AR.ImageResource(poiData.selectedImage);
        }
        this.markerDrawable_selected = new AR.ImageDrawable(World.markerDrawables[poiData.selectedImage], 2.5, {
            zOrder: 0,
            opacity: 0.0,
            onClick: onClick
        });
        cams.push(this.markerDrawable_selected);
    } else {
        this.markerDrawable_selected = new AR.ImageDrawable(World.markerDrawables['selected'], 2.5, {
            zOrder: 0,
            opacity: 0.0,
            onClick: onClick
        });
    ...
    */
    this.directionIndicatorDrawable = new AR.ImageDrawable(World.markerDrawables['direction'], 0.1, {
        enabled: false,
        verticalAnchor: AR.CONST.VERTICAL_ANCHOR.TOP
    });
...
Marker.prototype.setSelected = function(marker) {
    ...
    if (marker.animationGroup_selected === null) {
        ...
        group.push(new AR.PropertyAnimation(marker.markerDrawable_idle, "opacity", null, 0.0,
        ...
        group.push(new AR.PropertyAnimation(marker.markerDrawable_selected, "opacity",
            null, 1.0, kMarker_AnimationDuration_ChangeDrawable));

        // create AR.PropertyAnimation that animates the scaling of the idle-state-drawable to 1.2
        group.push(new AR.PropertyAnimation(marker.markerDrawable_idle, 'scaling', null, 1.2,
            kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                amplitude: 2.0
            })));
        // create AR.PropertyAnimation that animates the scaling of the selected-state-drawable to 1.2
        group.push(new AR.PropertyAnimation(marker.markerDrawable_selected, 'scaling',
            null, 1.2, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                amplitude: 2.0
            })));
        // create AR.PropertyAnimation that animates the scaling of the title label to 1.2
        group.push(new AR.PropertyAnimation(marker.titleLabel, 'scaling', null, 1.2,
            kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                amplitude: 2.0
            })));
        // create AR.PropertyAnimation that animates the scaling of the description label to 1.2
        if (marker.descriptionLabel) {
            group.push(new AR.PropertyAnimation(marker.descriptionLabel, 'scaling', null,
                1.2, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                    amplitude: 2.0
                })));
        }
        ...
        */
        marker.animationGroup_selected = new AR.AnimationGroup(AR.CONST.ANIMATION_GROUP_TYPE.PARALLEL, group);
    }
...
Marker.prototype.setDeselected = function(marker) {
    ...
    if (marker.animationGroup_idle === null) {
        ...
        group.push(new AR.PropertyAnimation(marker.markerDrawable_idle, "opacity", null, 1.0,
            kMarker_AnimationDuration_ChangeDrawable));
        // create AR.PropertyAnimation that animates the opacity to 0.0 in order to hide the selected-state-drawable
        group.push(new AR.PropertyAnimation(marker.markerDrawable_selected, "opacity",
            null, 0, kMarker_AnimationDuration_ChangeDrawable));
        // create AR.PropertyAnimation that animates the scaling of the idle-state-drawable to 1.0
        group.push(new AR.PropertyAnimation(marker.markerDrawable_idle, 'scaling', null, 1.0,
            kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                amplitude: 2.0
            })));
        // create AR.PropertyAnimation that animates the scaling of the selected-state-drawable to 1.0
        group.push(new AR.PropertyAnimation(marker.markerDrawable_selected, 'scaling',
            null, 1.0, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                amplitude: 2.0
            })));
        // create AR.PropertyAnimation that animates the scaling of the title label to 1.0
        group.push(new AR.PropertyAnimation(marker.titleLabel, 'scaling', null, 1.0,
            kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                amplitude: 2.0
            })));
        ...
        if (marker.descriptionLabel) {
            group.push(new AR.PropertyAnimation(marker.descriptionLabel, 'scaling', null,
                1.0, kMarker_AnimationDuration_Resize, new AR.EasingCurve(AR.CONST.EASING_CURVE_TYPE.EASE_OUT_ELASTIC, {
                    amplitude: 2.0
                })));
        }
        ...
        /*
            There are two types of AR.AnimationGroups. Parallel animations are running at the same time, sequentials are played one after another. This example uses a parallel AR.AnimationGroup.
        */
        marker.animationGroup_idle = new AR.AnimationGroup(AR.CONST.ANIMATION_GROUP_TYPE.PARALLEL, group);
    }
...
Marker.prototype.isAnyAnimationRunning = function(marker) {
    ...
    if (marker.animationGroup_idle === null || marker.animationGroup_selected === null) {
        ...
        if ((marker.animationGroup_idle.isRunning() === true) || (marker.animationGroup_selected.isRunning() ===
                true)) {
            return true;
        } else {
            return false;
